...
function cadastraHospede ($conexao, Hospede $hospede) {
	...
	$query = "insert into hospedes (nome, cpf, data_nascimento, sexo, telefone, celular, email, estado_civil, cep, rua, bairro, cidade, estado, data_checkin, data_checkout, qtd_diarias, qtd_pagantes, preco_diaria, valor_pago, preco_total, info_extras, categoria_id) values ('{$hospede->nome}', '{$hospede->cpf}', '{$hospede->dataNascimento}', '{$hospede->sexo}', '{$hospede->telefone}', '{$hospede->celular}', '{$hospede->email}', '{$hospede->estadoCivil}', '{$hospede->cep}', '{$hospede->rua}', '{$hospede->bairro}', '{$hospede->cidade}', '{$hospede->estado}', '{$hospede->dataCheckin}', '{$hospede->dataCheckout}', {$hospede->qtdDiarias}, {$hospede->qtdAcompanhantes}, {$hospede->precoDiaria}, {$hospede->valorPago}, {$hospede->precoTotal}, '{$hospede->infoExtras}', {$hospede->categoria->id})";
